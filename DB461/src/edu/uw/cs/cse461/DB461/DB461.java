package edu.uw.cs.cse461.DB461;
...
public abstract class DB461 {
	 ...
	 * @param <RecordType> The type of the Record for this table
	 */
	public abstract class DB461Table<K, RecordType extends Record> {
		final String    	mName;
		...
		public abstract RecordSet<RecordType> createRecordSet();  // new's up a RecordSet<R> 
		public abstract RecordType	    	 createRecord();     // new's up a Record of type R for the specific subclass table
		
		public abstract void write(RecordType record) throws DB461Exception;  // If record's key matches a record in the DB, record replaces it.
		 ...
		 */
		public RecordType readOne(K keyVal) throws DB461Exception {
			String query = "SELECT * FROM " + name() + " WHERE " + mKeyFieldName + "=";
			if ( keyVal.getClass() == String.class ) query += "'" + keyVal.toString() + "'";
			else query += keyVal;
			RecordSet<RecordType> result = (RecordSet<RecordType>)query(query);
		 ...
		 */
		public RecordSet<RecordType> readAll() throws DB461Exception {
			return (RecordSet<RecordType>)query("SELECT * FROM " + name());
		}
		 ...
		 * @throws DB461Exception
		 */
		protected RecordSet<RecordType> query(String query) throws DB461Exception {
			RecordSet<RecordType> result = createRecordSet();
			DBRecordIterator it = _queryTable(query);
			try {
				it = _queryTable(query);
				while(it.next()) {
					result.add( (RecordType)createRecord().initialize(it) );
				}
			} catch (Exception e) {
